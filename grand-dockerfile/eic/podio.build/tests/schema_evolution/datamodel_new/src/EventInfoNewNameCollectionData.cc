...
EventInfoNewNameCollectionData::EventInfoNewNameCollectionData() :
  m_data(new EventInfoNewNameDataContainer()) {
}

EventInfoNewNameCollectionData::EventInfoNewNameCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<EventInfoNewNameData>());
...
void EventInfoNewNameCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers EventInfoNewNameCollectionData::getCollectionBuffers(bool isSubsetColl) {
return {
...
void EventInfoNewNameCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) { pointer->clear(); }
...
void EventInfoNewNameCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new EventInfoNewNameObj({index, collectionID}, data);
...
bool EventInfoNewNameCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider, bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      EventInfoNewNameObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<EventInfoNewNameCollection*>(coll);
...
void EventInfoNewNameCollectionData::makeSubsetCollection() {
