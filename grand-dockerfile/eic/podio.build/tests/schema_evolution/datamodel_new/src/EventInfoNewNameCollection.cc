...
EventInfoNewNameCollection::EventInfoNewNameCollection() :
...
EventInfoNewNameCollection::EventInfoNewNameCollection(EventInfoNewNameCollectionData&& data, bool isSubsetColl) :
...
EventInfoNewName EventInfoNewNameCollection::operator[](std::size_t index) const {
  return EventInfoNewName(m_storage.entries[index]);
}

EventInfoNewName EventInfoNewNameCollection::at(std::size_t index) const {
  return EventInfoNewName(m_storage.entries.at(index));
}

MutableEventInfoNewName EventInfoNewNameCollection::operator[](std::size_t index) {
  return MutableEventInfoNewName(podio::utils::MaybeSharedPtr(m_storage.entries[index]));
}

MutableEventInfoNewName EventInfoNewNameCollection::at(std::size_t index) {
  return MutableEventInfoNewName(podio::utils::MaybeSharedPtr(m_storage.entries.at(index)));
}

std::size_t EventInfoNewNameCollection::size() const {
  return m_storage.entries.size();
}

bool EventInfoNewNameCollection::empty() const {
  return m_storage.entries.empty();
}

void EventInfoNewNameCollection::setSubsetCollection(bool setSubset) {
...
MutableEventInfoNewName EventInfoNewNameCollection::create() {
  if (m_isSubsetColl) {
    throw std::logic_error("Cannot create new elements on a subset collection");
  }

  auto obj = m_storage.entries.emplace_back(new EventInfoNewNameObj());

  obj->id = {int(m_storage.entries.size() - 1), m_collectionID};
  return MutableEventInfoNewName(podio::utils::MaybeSharedPtr(obj));
}

void EventInfoNewNameCollection::clear() {
  m_storage.clear(m_isSubsetColl);
  m_isPrepared = false;
}

void EventInfoNewNameCollection::prepareForWrite() const {
...
void EventInfoNewNameCollection::prepareAfterRead() {
...
bool EventInfoNewNameCollection::setReferences(const podio::ICollectionProvider* collectionProvider) {
  return m_storage.setReferences(collectionProvider, m_isSubsetColl);
}

void EventInfoNewNameCollection::push_back(MutableEventInfoNewName object) {
  ...
  if (!m_isSubsetColl) {
    ...
    push_back(EventInfoNewName(object));
  }
}

void EventInfoNewNameCollection::push_back(EventInfoNewName object) {
  if (!m_isSubsetColl) {
...
podio::CollectionWriteBuffers EventInfoNewNameCollection::getBuffers() {
  return m_storage.getCollectionBuffers(m_isSubsetColl);
}

std::vector<int> EventInfoNewNameCollection::Number(const size_t nElem) const {
  std::vector<int> tmp;
...
size_t EventInfoNewNameCollection::getDatamodelRegistryIndex() const {
  return datamodel::meta::DatamodelRegistryIndex::value();
}

podio::SchemaVersionT EventInfoNewNameCollection::getSchemaVersion() const {
...
podio::CollectionReadBuffers createBuffers(bool isSubset) {
  ...
  readBuffers.type = "EventInfoNewNameCollection";
  readBuffers.schemaVersion = datamodel::meta::schemaVersion;
  readBuffers.data = isSubset ? nullptr : new EventInfoNewNameDataContainer;
  ...
  readBuffers.createCollection = [](podio::CollectionReadBuffers buffers, bool isSubsetColl) {
    EventInfoNewNameCollectionData data(buffers, isSubsetColl);
    return std::make_unique<EventInfoNewNameCollection>(std::move(data), isSubsetColl);
  };

  readBuffers.recast = [](podio::CollectionReadBuffers& buffers) {
    // We only have any of these buffers if this is not a subset collection
    if (buffers.data) {
      buffers.data = podio::CollectionWriteBuffers::asVector<EventInfoNewNameData>(buffers.data);
    }
  };
  ...
  readBuffers.deleteBuffers = [](podio::CollectionReadBuffers& buffers) {
    if (buffers.data) {
      ...
      // we can delete
      delete static_cast<EventInfoNewNameDataContainer*>(buffers.data);
...
bool registerCollection() {
  const static auto reg = []() {
    ...
    factory.registerCreationFunc("EventInfoNewNameCollection", datamodel::meta::schemaVersion, createBuffers);
...
EventInfoNewName EventInfoNewNameCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<EventInfoNewNameObj>((*m_collection)[m_index]);
  return m_object;
}
...
EventInfoNewNameCollectionIterator& EventInfoNewNameCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
MutableEventInfoNewName EventInfoNewNameMutableCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<EventInfoNewNameObj>((*m_collection)[m_index]);
  return m_object;
}
...
EventInfoNewNameMutableCollectionIterator& EventInfoNewNameMutableCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
void EventInfoNewNameCollection::print(std::ostream& os, bool flush) const {
  os << *this;
  if (flush) {
    os.flush();
  }
