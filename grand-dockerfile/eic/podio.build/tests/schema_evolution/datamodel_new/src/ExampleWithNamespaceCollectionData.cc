...
ExampleWithNamespaceCollectionData::ExampleWithNamespaceCollectionData() :
  m_data(new ExampleWithNamespaceDataContainer()) {
}

ExampleWithNamespaceCollectionData::ExampleWithNamespaceCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<ex42::ExampleWithNamespaceData>());
...
void ExampleWithNamespaceCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers ExampleWithNamespaceCollectionData::getCollectionBuffers(bool isSubsetColl) {
return {
...
void ExampleWithNamespaceCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) { pointer->clear(); }
...
void ExampleWithNamespaceCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new ExampleWithNamespaceObj({index, collectionID}, data);
...
bool ExampleWithNamespaceCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider, bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      ex42::ExampleWithNamespaceObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<ex42::ExampleWithNamespaceCollection*>(coll);
...
void ExampleWithNamespaceCollectionData::makeSubsetCollection() {
