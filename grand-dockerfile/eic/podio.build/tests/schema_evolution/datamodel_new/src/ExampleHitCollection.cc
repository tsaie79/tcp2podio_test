...
ExampleHitCollection::ExampleHitCollection(ExampleHitCollectionData&& data, bool isSubsetColl) :
...
ExampleHit ExampleHitCollection::operator[](std::size_t index) const {
  return ExampleHit(m_storage.entries[index]);
}

ExampleHit ExampleHitCollection::at(std::size_t index) const {
  return ExampleHit(m_storage.entries.at(index));
}

MutableExampleHit ExampleHitCollection::operator[](std::size_t index) {
  return MutableExampleHit(podio::utils::MaybeSharedPtr(m_storage.entries[index]));
}

MutableExampleHit ExampleHitCollection::at(std::size_t index) {
  return MutableExampleHit(podio::utils::MaybeSharedPtr(m_storage.entries.at(index)));
}

std::size_t ExampleHitCollection::size() const {
  return m_storage.entries.size();
}

bool ExampleHitCollection::empty() const {
  return m_storage.entries.empty();
}

void ExampleHitCollection::setSubsetCollection(bool setSubset) {
...
MutableExampleHit ExampleHitCollection::create() {
  if (m_isSubsetColl) {
    throw std::logic_error("Cannot create new elements on a subset collection");
  }

  auto obj = m_storage.entries.emplace_back(new ExampleHitObj());
...
void ExampleHitCollection::prepareForWrite() const {
...
void ExampleHitCollection::push_back(MutableExampleHit object) {
...
podio::CollectionWriteBuffers ExampleHitCollection::getBuffers() {
  return m_storage.getCollectionBuffers(m_isSubsetColl);
}

std::vector<unsigned long long> ExampleHitCollection::cellID(const size_t nElem) const {
...
std::vector<double> ExampleHitCollection::x(const size_t nElem) const {
...
std::vector<double> ExampleHitCollection::y(const size_t nElem) const {
  std::vector<double> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.y);
  }
  return tmp;
}

std::vector<double> ExampleHitCollection::z(const size_t nElem) const {
  std::vector<double> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.z);
  }
  return tmp;
}

std::vector<double> ExampleHitCollection::energy(const size_t nElem) const {
  std::vector<double> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
...
std::vector<double> ExampleHitCollection::t(const size_t nElem) const {
  std::vector<double> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.t);
  }
  return tmp;
}


size_t ExampleHitCollection::getDatamodelRegistryIndex() const {
  return datamodel::meta::DatamodelRegistryIndex::value();
}

podio::SchemaVersionT ExampleHitCollection::getSchemaVersion() const {
...
podio::CollectionReadBuffers createBuffers(bool isSubset) {
  ...
  readBuffers.type = "ExampleHitCollection";
  readBuffers.schemaVersion = datamodel::meta::schemaVersion;
  readBuffers.data = isSubset ? nullptr : new ExampleHitDataContainer;
  ...
  readBuffers.createCollection = [](podio::CollectionReadBuffers buffers, bool isSubsetColl) {
    ExampleHitCollectionData data(buffers, isSubsetColl);
    return std::make_unique<ExampleHitCollection>(std::move(data), isSubsetColl);
  };

  readBuffers.recast = [](podio::CollectionReadBuffers& buffers) {
    // We only have any of these buffers if this is not a subset collection
    if (buffers.data) {
      buffers.data = podio::CollectionWriteBuffers::asVector<ExampleHitData>(buffers.data);
    }
  };
  ...
  readBuffers.deleteBuffers = [](podio::CollectionReadBuffers& buffers) {
    if (buffers.data) {
      ...
      // we can delete
      delete static_cast<ExampleHitDataContainer*>(buffers.data);
...
bool registerCollection() {
  const static auto reg = []() {
    ...
    factory.registerCreationFunc("ExampleHitCollection", datamodel::meta::schemaVersion, createBuffers);
...
ExampleHit ExampleHitCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<ExampleHitObj>((*m_collection)[m_index]);
  return m_object;
}
...
ExampleHitCollectionIterator& ExampleHitCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
MutableExampleHit ExampleHitMutableCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<ExampleHitObj>((*m_collection)[m_index]);
  return m_object;
}
...
ExampleHitMutableCollectionIterator& ExampleHitMutableCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
std::ostream& operator<<(std::ostream& o, const ExampleHitCollection& v) {
  ...
  o << "          id:      cellID:           x:           y:           z:      energy:           t:" << '\n';

  for (const auto&& el : v) {
    o << std::scientific << std::showpos << std::setw(12) << el.id() << " "
      << std::setw(12) << el.cellID() << " "
      << std::setw(12) << el.x() << " "
      << std::setw(12) << el.y() << " "
      << std::setw(12) << el.z() << " "
...
void ExampleHitCollection::print(std::ostream& os, bool flush) const {
  os << *this;
  if (flush) {
    os.flush();
  }
