...
ExampleWithArrayCollection::ExampleWithArrayCollection(ExampleWithArrayCollectionData&& data, bool isSubsetColl) :
...
ExampleWithArray ExampleWithArrayCollection::operator[](std::size_t index) const {
  return ExampleWithArray(m_storage.entries[index]);
}

ExampleWithArray ExampleWithArrayCollection::at(std::size_t index) const {
  return ExampleWithArray(m_storage.entries.at(index));
}

MutableExampleWithArray ExampleWithArrayCollection::operator[](std::size_t index) {
  return MutableExampleWithArray(podio::utils::MaybeSharedPtr(m_storage.entries[index]));
}

MutableExampleWithArray ExampleWithArrayCollection::at(std::size_t index) {
  return MutableExampleWithArray(podio::utils::MaybeSharedPtr(m_storage.entries.at(index)));
}

std::size_t ExampleWithArrayCollection::size() const {
  return m_storage.entries.size();
}

bool ExampleWithArrayCollection::empty() const {
  return m_storage.entries.empty();
}

void ExampleWithArrayCollection::setSubsetCollection(bool setSubset) {
...
MutableExampleWithArray ExampleWithArrayCollection::create() {
  if (m_isSubsetColl) {
    throw std::logic_error("Cannot create new elements on a subset collection");
  }

  auto obj = m_storage.entries.emplace_back(new ExampleWithArrayObj());
...
void ExampleWithArrayCollection::prepareForWrite() const {
...
void ExampleWithArrayCollection::push_back(MutableExampleWithArray object) {
  ...
  if (!m_isSubsetColl) {
    ...
    push_back(ExampleWithArray(object));
  }
}

void ExampleWithArrayCollection::push_back(ExampleWithArray object) {
  if (!m_isSubsetColl) {
...
podio::CollectionWriteBuffers ExampleWithArrayCollection::getBuffers() {
  return m_storage.getCollectionBuffers(m_isSubsetColl);
}

std::vector<NotSoSimpleStruct> ExampleWithArrayCollection::arrayStruct(const size_t nElem) const {
  ...
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.arrayStruct);
  }
  return tmp;
}

std::vector<std::array<int, 4>> ExampleWithArrayCollection::myArray(const size_t nElem) const {
  std::vector<std::array<int, 4>> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.myArray);
  }
  return tmp;
}

std::vector<std::array<int, 4>> ExampleWithArrayCollection::anotherArray2(const size_t nElem) const {
  std::vector<std::array<int, 4>> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.anotherArray2);
  }
  return tmp;
}

std::vector<std::array<int, 4>> ExampleWithArrayCollection::snail_case_array(const size_t nElem) const {
  std::vector<std::array<int, 4>> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.snail_case_array);
  }
  return tmp;
}

std::vector<std::array<int, 4>> ExampleWithArrayCollection::snail_case_Array3(const size_t nElem) const {
  std::vector<std::array<int, 4>> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.snail_case_Array3);
  }
  return tmp;
}

std::vector<std::array<ex2::NamespaceStruct, 4>> ExampleWithArrayCollection::structArray(const size_t nElem) const {
  std::vector<std::array<ex2::NamespaceStruct, 4>> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.structArray);
  }
  return tmp;
}


size_t ExampleWithArrayCollection::getDatamodelRegistryIndex() const {
  return datamodel::meta::DatamodelRegistryIndex::value();
}

podio::SchemaVersionT ExampleWithArrayCollection::getSchemaVersion() const {
...
podio::CollectionReadBuffers createBuffers(bool isSubset) {
  ...
  readBuffers.type = "ExampleWithArrayCollection";
  readBuffers.schemaVersion = datamodel::meta::schemaVersion;
  readBuffers.data = isSubset ? nullptr : new ExampleWithArrayDataContainer;
  ...
  readBuffers.createCollection = [](podio::CollectionReadBuffers buffers, bool isSubsetColl) {
    ExampleWithArrayCollectionData data(buffers, isSubsetColl);
    return std::make_unique<ExampleWithArrayCollection>(std::move(data), isSubsetColl);
  };

  readBuffers.recast = [](podio::CollectionReadBuffers& buffers) {
    // We only have any of these buffers if this is not a subset collection
    if (buffers.data) {
      buffers.data = podio::CollectionWriteBuffers::asVector<ExampleWithArrayData>(buffers.data);
    }
  };
  ...
  readBuffers.deleteBuffers = [](podio::CollectionReadBuffers& buffers) {
    if (buffers.data) {
      ...
      // we can delete
      delete static_cast<ExampleWithArrayDataContainer*>(buffers.data);
...
bool registerCollection() {
  const static auto reg = []() {
    ...
    factory.registerCreationFunc("ExampleWithArrayCollection", datamodel::meta::schemaVersion, createBuffers);
...
ExampleWithArray ExampleWithArrayCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<ExampleWithArrayObj>((*m_collection)[m_index]);
  return m_object;
}
...
ExampleWithArrayCollectionIterator& ExampleWithArrayCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
MutableExampleWithArray ExampleWithArrayMutableCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<ExampleWithArrayObj>((*m_collection)[m_index]);
  return m_object;
}
...
ExampleWithArrayMutableCollectionIterator& ExampleWithArrayMutableCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
void ExampleWithArrayCollection::print(std::ostream& os, bool flush) const {
  os << *this;
  if (flush) {
    os.flush();
  }
