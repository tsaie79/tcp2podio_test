...
ExampleWithArrayComponentCollectionData::ExampleWithArrayComponentCollectionData() :
  m_data(new ExampleWithArrayComponentDataContainer()) {
}

ExampleWithArrayComponentCollectionData::ExampleWithArrayComponentCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<ExampleWithArrayComponentData>());
...
void ExampleWithArrayComponentCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers ExampleWithArrayComponentCollectionData::getCollectionBuffers(bool isSubsetColl) {
return {
...
void ExampleWithArrayComponentCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) { pointer->clear(); }
...
void ExampleWithArrayComponentCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new ExampleWithArrayComponentObj({index, collectionID}, data);
...
bool ExampleWithArrayComponentCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider, bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      ExampleWithArrayComponentObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<ExampleWithArrayComponentCollection*>(coll);
...
void ExampleWithArrayComponentCollectionData::makeSubsetCollection() {
