...
ExampleForCyclicDependency1CollectionData::ExampleForCyclicDependency1CollectionData() :
  m_rel_ref(new std::vector<::ExampleForCyclicDependency2>()),
...
ExampleForCyclicDependency1CollectionData::ExampleForCyclicDependency1CollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  m_rel_ref(new std::vector<::ExampleForCyclicDependency2>()),
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<ExampleForCyclicDependency1Data>());
...
void ExampleForCyclicDependency1CollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers ExampleForCyclicDependency1CollectionData::getCollectionBuffers(bool isSubsetColl) {
return {
...
void ExampleForCyclicDependency1CollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) { pointer->clear(); }
...
void ExampleForCyclicDependency1CollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new ExampleForCyclicDependency1Obj({index, collectionID}, data);
...
bool ExampleForCyclicDependency1CollectionData::setReferences(const podio::ICollectionProvider* collectionProvider, bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      ExampleForCyclicDependency1Obj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<ExampleForCyclicDependency1Collection*>(coll);
  ...
  for (unsigned int i = 0, size = entries.size(); i != size; ++i) {
    ...
    if (id.index != podio::ObjectID::invalid) {
      ...
      entries[i]->m_ref = new ExampleForCyclicDependency2((*tmp_coll)[id.index]);
    } else {
      entries[i]->m_ref = nullptr;
    }
  }
...
void ExampleForCyclicDependency1CollectionData::makeSubsetCollection() {
