...
ExampleWithFixedWidthIntegersCollectionData::ExampleWithFixedWidthIntegersCollectionData() :
  m_data(new ExampleWithFixedWidthIntegersDataContainer()) {
}

ExampleWithFixedWidthIntegersCollectionData::ExampleWithFixedWidthIntegersCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<ExampleWithFixedWidthIntegersData>());
...
void ExampleWithFixedWidthIntegersCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers ExampleWithFixedWidthIntegersCollectionData::getCollectionBuffers(bool isSubsetColl) {
return {
...
void ExampleWithFixedWidthIntegersCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) { pointer->clear(); }
...
void ExampleWithFixedWidthIntegersCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new ExampleWithFixedWidthIntegersObj({index, collectionID}, data);
...
bool ExampleWithFixedWidthIntegersCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider, bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      ExampleWithFixedWidthIntegersObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<ExampleWithFixedWidthIntegersCollection*>(coll);
...
void ExampleWithFixedWidthIntegersCollectionData::makeSubsetCollection() {
