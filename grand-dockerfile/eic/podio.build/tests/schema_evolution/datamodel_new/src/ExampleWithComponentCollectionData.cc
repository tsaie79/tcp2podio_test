...
ExampleWithComponentCollectionData::ExampleWithComponentCollectionData() :
  m_data(new ExampleWithComponentDataContainer()) {
}

ExampleWithComponentCollectionData::ExampleWithComponentCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<ExampleWithComponentData>());
...
void ExampleWithComponentCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers ExampleWithComponentCollectionData::getCollectionBuffers(bool isSubsetColl) {
return {
...
void ExampleWithComponentCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) { pointer->clear(); }
...
void ExampleWithComponentCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new ExampleWithComponentObj({index, collectionID}, data);
...
bool ExampleWithComponentCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider, bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      ExampleWithComponentObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<ExampleWithComponentCollection*>(coll);
...
void ExampleWithComponentCollectionData::makeSubsetCollection() {
