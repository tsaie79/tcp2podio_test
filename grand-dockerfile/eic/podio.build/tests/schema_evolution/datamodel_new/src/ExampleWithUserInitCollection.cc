...
ExampleWithUserInitCollection::ExampleWithUserInitCollection() :
...
ExampleWithUserInitCollection::ExampleWithUserInitCollection(ExampleWithUserInitCollectionData&& data, bool isSubsetColl) :
...
ExampleWithUserInit ExampleWithUserInitCollection::operator[](std::size_t index) const {
  return ExampleWithUserInit(m_storage.entries[index]);
}

ExampleWithUserInit ExampleWithUserInitCollection::at(std::size_t index) const {
  return ExampleWithUserInit(m_storage.entries.at(index));
}

MutableExampleWithUserInit ExampleWithUserInitCollection::operator[](std::size_t index) {
  return MutableExampleWithUserInit(podio::utils::MaybeSharedPtr(m_storage.entries[index]));
}

MutableExampleWithUserInit ExampleWithUserInitCollection::at(std::size_t index) {
  return MutableExampleWithUserInit(podio::utils::MaybeSharedPtr(m_storage.entries.at(index)));
}

std::size_t ExampleWithUserInitCollection::size() const {
  return m_storage.entries.size();
}

bool ExampleWithUserInitCollection::empty() const {
  return m_storage.entries.empty();
}

void ExampleWithUserInitCollection::setSubsetCollection(bool setSubset) {
...
MutableExampleWithUserInit ExampleWithUserInitCollection::create() {
  if (m_isSubsetColl) {
    throw std::logic_error("Cannot create new elements on a subset collection");
  }

  auto obj = m_storage.entries.emplace_back(new ExampleWithUserInitObj());

  obj->id = {int(m_storage.entries.size() - 1), m_collectionID};
  return MutableExampleWithUserInit(podio::utils::MaybeSharedPtr(obj));
}

void ExampleWithUserInitCollection::clear() {
  m_storage.clear(m_isSubsetColl);
  m_isPrepared = false;
}

void ExampleWithUserInitCollection::prepareForWrite() const {
...
void ExampleWithUserInitCollection::prepareAfterRead() {
...
bool ExampleWithUserInitCollection::setReferences(const podio::ICollectionProvider* collectionProvider) {
  return m_storage.setReferences(collectionProvider, m_isSubsetColl);
}

void ExampleWithUserInitCollection::push_back(MutableExampleWithUserInit object) {
  ...
  if (!m_isSubsetColl) {
    ...
    push_back(ExampleWithUserInit(object));
  }
}

void ExampleWithUserInitCollection::push_back(ExampleWithUserInit object) {
  if (!m_isSubsetColl) {
...
podio::CollectionWriteBuffers ExampleWithUserInitCollection::getBuffers() {
  return m_storage.getCollectionBuffers(m_isSubsetColl);
}

std::vector<std::int16_t> ExampleWithUserInitCollection::i16Val(const size_t nElem) const {
  ...
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.i16Val);
  }
  return tmp;
}

std::vector<std::array<float, 2>> ExampleWithUserInitCollection::floats(const size_t nElem) const {
  std::vector<std::array<float, 2>> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.floats);
  }
  return tmp;
}

std::vector<ex2::NamespaceStruct> ExampleWithUserInitCollection::s(const size_t nElem) const {
  std::vector<ex2::NamespaceStruct> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.s);
  }
  return tmp;
}

std::vector<double> ExampleWithUserInitCollection::d(const size_t nElem) const {
  std::vector<double> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.d);
  }
  return tmp;
}

std::vector<CompWithInit> ExampleWithUserInitCollection::comp(const size_t nElem) const {
  std::vector<CompWithInit> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.comp);
  }
  return tmp;
}


size_t ExampleWithUserInitCollection::getDatamodelRegistryIndex() const {
  return datamodel::meta::DatamodelRegistryIndex::value();
}

podio::SchemaVersionT ExampleWithUserInitCollection::getSchemaVersion() const {
...
podio::CollectionReadBuffers createBuffers(bool isSubset) {
  ...
  readBuffers.type = "ExampleWithUserInitCollection";
  readBuffers.schemaVersion = datamodel::meta::schemaVersion;
  readBuffers.data = isSubset ? nullptr : new ExampleWithUserInitDataContainer;
  ...
  readBuffers.createCollection = [](podio::CollectionReadBuffers buffers, bool isSubsetColl) {
    ExampleWithUserInitCollectionData data(buffers, isSubsetColl);
    return std::make_unique<ExampleWithUserInitCollection>(std::move(data), isSubsetColl);
  };

  readBuffers.recast = [](podio::CollectionReadBuffers& buffers) {
    // We only have any of these buffers if this is not a subset collection
    if (buffers.data) {
      buffers.data = podio::CollectionWriteBuffers::asVector<ExampleWithUserInitData>(buffers.data);
    }
  };
  ...
  readBuffers.deleteBuffers = [](podio::CollectionReadBuffers& buffers) {
    if (buffers.data) {
      ...
      // we can delete
      delete static_cast<ExampleWithUserInitDataContainer*>(buffers.data);
...
bool registerCollection() {
  const static auto reg = []() {
    ...
    factory.registerCreationFunc("ExampleWithUserInitCollection", datamodel::meta::schemaVersion, createBuffers);
...
ExampleWithUserInit ExampleWithUserInitCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<ExampleWithUserInitObj>((*m_collection)[m_index]);
  return m_object;
}
...
ExampleWithUserInitCollectionIterator& ExampleWithUserInitCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
MutableExampleWithUserInit ExampleWithUserInitMutableCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<ExampleWithUserInitObj>((*m_collection)[m_index]);
  return m_object;
}
...
ExampleWithUserInitMutableCollectionIterator& ExampleWithUserInitMutableCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
std::ostream& operator<<(std::ostream& o, const ExampleWithUserInitCollection& v) {
  ...
  o << "          id:      i16Val:      floats:           s [ x, y_new]:           d:          comp [ i, arr]:" << '\n';

  for (const auto&& el : v) {
    o << std::scientific << std::showpos << std::setw(12) << el.id() << " "
      << std::setw(12) << el.i16Val() << " "
      << std::setw(12) << el.s() << " "
      << std::setw(12) << el.d() << " "
      << std::setw(12) << el.comp() << " "
      << std::endl;
...
void ExampleWithUserInitCollection::print(std::ostream& os, bool flush) const {
  os << *this;
  if (flush) {
    os.flush();
  }
