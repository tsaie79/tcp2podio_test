...
ExampleOfDroppedTypeCollectionData::ExampleOfDroppedTypeCollectionData() :
  m_data(new ExampleOfDroppedTypeDataContainer()) {
}

ExampleOfDroppedTypeCollectionData::ExampleOfDroppedTypeCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<ExampleOfDroppedTypeData>());
...
void ExampleOfDroppedTypeCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers ExampleOfDroppedTypeCollectionData::getCollectionBuffers(bool isSubsetColl) {
return {
...
void ExampleOfDroppedTypeCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) { pointer->clear(); }
...
void ExampleOfDroppedTypeCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new ExampleOfDroppedTypeObj({index, collectionID}, data);
...
bool ExampleOfDroppedTypeCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider, bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      ExampleOfDroppedTypeObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<ExampleOfDroppedTypeCollection*>(coll);
...
void ExampleOfDroppedTypeCollectionData::makeSubsetCollection() {
