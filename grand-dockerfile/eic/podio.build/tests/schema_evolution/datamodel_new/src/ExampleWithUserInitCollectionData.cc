...
ExampleWithUserInitCollectionData::ExampleWithUserInitCollectionData() :
  m_data(new ExampleWithUserInitDataContainer()) {
}

ExampleWithUserInitCollectionData::ExampleWithUserInitCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<ExampleWithUserInitData>());
...
void ExampleWithUserInitCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers ExampleWithUserInitCollectionData::getCollectionBuffers(bool isSubsetColl) {
return {
...
void ExampleWithUserInitCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) { pointer->clear(); }
...
void ExampleWithUserInitCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new ExampleWithUserInitObj({index, collectionID}, data);
...
bool ExampleWithUserInitCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider, bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      ExampleWithUserInitObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<ExampleWithUserInitCollection*>(coll);
...
void ExampleWithUserInitCollectionData::makeSubsetCollection() {
