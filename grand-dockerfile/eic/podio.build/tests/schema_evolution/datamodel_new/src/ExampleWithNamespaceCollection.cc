...
ExampleWithNamespaceCollection::ExampleWithNamespaceCollection(ExampleWithNamespaceCollectionData&& data, bool isSubsetColl) :
...
ExampleWithNamespace ExampleWithNamespaceCollection::operator[](std::size_t index) const {
  return ExampleWithNamespace(m_storage.entries[index]);
}

ExampleWithNamespace ExampleWithNamespaceCollection::at(std::size_t index) const {
  return ExampleWithNamespace(m_storage.entries.at(index));
}

MutableExampleWithNamespace ExampleWithNamespaceCollection::operator[](std::size_t index) {
  return MutableExampleWithNamespace(podio::utils::MaybeSharedPtr(m_storage.entries[index]));
}

MutableExampleWithNamespace ExampleWithNamespaceCollection::at(std::size_t index) {
  return MutableExampleWithNamespace(podio::utils::MaybeSharedPtr(m_storage.entries.at(index)));
}

std::size_t ExampleWithNamespaceCollection::size() const {
  return m_storage.entries.size();
}

bool ExampleWithNamespaceCollection::empty() const {
  return m_storage.entries.empty();
}

void ExampleWithNamespaceCollection::setSubsetCollection(bool setSubset) {
...
MutableExampleWithNamespace ExampleWithNamespaceCollection::create() {
  if (m_isSubsetColl) {
    throw std::logic_error("Cannot create new elements on a subset collection");
  }

  auto obj = m_storage.entries.emplace_back(new ExampleWithNamespaceObj());
...
void ExampleWithNamespaceCollection::prepareForWrite() const {
...
void ExampleWithNamespaceCollection::push_back(MutableExampleWithNamespace object) {
  ...
  if (!m_isSubsetColl) {
    ...
    push_back(ExampleWithNamespace(object));
  }
}
...
podio::CollectionWriteBuffers ExampleWithNamespaceCollection::getBuffers() {
  return m_storage.getCollectionBuffers(m_isSubsetColl);
}

std::vector<ex2::NamespaceStruct> ExampleWithNamespaceCollection::component(const size_t nElem) const {
...
size_t ExampleWithNamespaceCollection::getDatamodelRegistryIndex() const {
  return datamodel::meta::DatamodelRegistryIndex::value();
}

podio::SchemaVersionT ExampleWithNamespaceCollection::getSchemaVersion() const {
...
podio::CollectionReadBuffers createBuffers(bool isSubset) {
  ...
  readBuffers.type = "ex42::ExampleWithNamespaceCollection";
  readBuffers.schemaVersion = datamodel::meta::schemaVersion;
  readBuffers.data = isSubset ? nullptr : new ExampleWithNamespaceDataContainer;
  ...
  readBuffers.createCollection = [](podio::CollectionReadBuffers buffers, bool isSubsetColl) {
    ExampleWithNamespaceCollectionData data(buffers, isSubsetColl);
    return std::make_unique<ExampleWithNamespaceCollection>(std::move(data), isSubsetColl);
  };

  readBuffers.recast = [](podio::CollectionReadBuffers& buffers) {
    // We only have any of these buffers if this is not a subset collection
    if (buffers.data) {
      buffers.data = podio::CollectionWriteBuffers::asVector<ex42::ExampleWithNamespaceData>(buffers.data);
    }
  };
  ...
  readBuffers.deleteBuffers = [](podio::CollectionReadBuffers& buffers) {
    if (buffers.data) {
      ...
      // we can delete
      delete static_cast<ex42::ExampleWithNamespaceDataContainer*>(buffers.data);
...
bool registerCollection() {
  const static auto reg = []() {
    ...
    factory.registerCreationFunc("ex42::ExampleWithNamespaceCollection", datamodel::meta::schemaVersion, createBuffers);
...
ExampleWithNamespace ExampleWithNamespaceCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<ExampleWithNamespaceObj>((*m_collection)[m_index]);
  return m_object;
}
...
ExampleWithNamespaceCollectionIterator& ExampleWithNamespaceCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
MutableExampleWithNamespace ExampleWithNamespaceMutableCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<ExampleWithNamespaceObj>((*m_collection)[m_index]);
  return m_object;
}
...
ExampleWithNamespaceMutableCollectionIterator& ExampleWithNamespaceMutableCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
void ExampleWithNamespaceCollection::print(std::ostream& os, bool flush) const {
  os << *this;
  if (flush) {
    os.flush();
  }
