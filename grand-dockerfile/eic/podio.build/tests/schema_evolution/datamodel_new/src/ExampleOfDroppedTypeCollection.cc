...
ExampleOfDroppedTypeCollection::ExampleOfDroppedTypeCollection() :
...
ExampleOfDroppedTypeCollection::ExampleOfDroppedTypeCollection(ExampleOfDroppedTypeCollectionData&& data, bool isSubsetColl) :
...
ExampleOfDroppedType ExampleOfDroppedTypeCollection::operator[](std::size_t index) const {
  return ExampleOfDroppedType(m_storage.entries[index]);
}

ExampleOfDroppedType ExampleOfDroppedTypeCollection::at(std::size_t index) const {
  return ExampleOfDroppedType(m_storage.entries.at(index));
}

MutableExampleOfDroppedType ExampleOfDroppedTypeCollection::operator[](std::size_t index) {
  return MutableExampleOfDroppedType(podio::utils::MaybeSharedPtr(m_storage.entries[index]));
}

MutableExampleOfDroppedType ExampleOfDroppedTypeCollection::at(std::size_t index) {
  return MutableExampleOfDroppedType(podio::utils::MaybeSharedPtr(m_storage.entries.at(index)));
}

std::size_t ExampleOfDroppedTypeCollection::size() const {
  return m_storage.entries.size();
}

bool ExampleOfDroppedTypeCollection::empty() const {
  return m_storage.entries.empty();
}

void ExampleOfDroppedTypeCollection::setSubsetCollection(bool setSubset) {
...
MutableExampleOfDroppedType ExampleOfDroppedTypeCollection::create() {
  if (m_isSubsetColl) {
    throw std::logic_error("Cannot create new elements on a subset collection");
  }

  auto obj = m_storage.entries.emplace_back(new ExampleOfDroppedTypeObj());

  obj->id = {int(m_storage.entries.size() - 1), m_collectionID};
  return MutableExampleOfDroppedType(podio::utils::MaybeSharedPtr(obj));
}

void ExampleOfDroppedTypeCollection::clear() {
  m_storage.clear(m_isSubsetColl);
  m_isPrepared = false;
}

void ExampleOfDroppedTypeCollection::prepareForWrite() const {
...
void ExampleOfDroppedTypeCollection::prepareAfterRead() {
...
bool ExampleOfDroppedTypeCollection::setReferences(const podio::ICollectionProvider* collectionProvider) {
  return m_storage.setReferences(collectionProvider, m_isSubsetColl);
}

void ExampleOfDroppedTypeCollection::push_back(MutableExampleOfDroppedType object) {
  ...
  if (!m_isSubsetColl) {
    ...
    push_back(ExampleOfDroppedType(object));
  }
}

void ExampleOfDroppedTypeCollection::push_back(ExampleOfDroppedType object) {
  if (!m_isSubsetColl) {
...
podio::CollectionWriteBuffers ExampleOfDroppedTypeCollection::getBuffers() {
  return m_storage.getCollectionBuffers(m_isSubsetColl);
}

std::vector<int> ExampleOfDroppedTypeCollection::x(const size_t nElem) const {
  std::vector<int> tmp;
...
size_t ExampleOfDroppedTypeCollection::getDatamodelRegistryIndex() const {
  return datamodel::meta::DatamodelRegistryIndex::value();
}

podio::SchemaVersionT ExampleOfDroppedTypeCollection::getSchemaVersion() const {
...
podio::CollectionReadBuffers createBuffers(bool isSubset) {
  ...
  readBuffers.type = "ExampleOfDroppedTypeCollection";
  readBuffers.schemaVersion = datamodel::meta::schemaVersion;
  readBuffers.data = isSubset ? nullptr : new ExampleOfDroppedTypeDataContainer;
  ...
  readBuffers.createCollection = [](podio::CollectionReadBuffers buffers, bool isSubsetColl) {
    ExampleOfDroppedTypeCollectionData data(buffers, isSubsetColl);
    return std::make_unique<ExampleOfDroppedTypeCollection>(std::move(data), isSubsetColl);
  };

  readBuffers.recast = [](podio::CollectionReadBuffers& buffers) {
    // We only have any of these buffers if this is not a subset collection
    if (buffers.data) {
      buffers.data = podio::CollectionWriteBuffers::asVector<ExampleOfDroppedTypeData>(buffers.data);
    }
  };
  ...
  readBuffers.deleteBuffers = [](podio::CollectionReadBuffers& buffers) {
    if (buffers.data) {
      ...
      // we can delete
      delete static_cast<ExampleOfDroppedTypeDataContainer*>(buffers.data);
...
bool registerCollection() {
  const static auto reg = []() {
    ...
    factory.registerCreationFunc("ExampleOfDroppedTypeCollection", datamodel::meta::schemaVersion, createBuffers);
...
ExampleOfDroppedType ExampleOfDroppedTypeCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<ExampleOfDroppedTypeObj>((*m_collection)[m_index]);
  return m_object;
}
...
ExampleOfDroppedTypeCollectionIterator& ExampleOfDroppedTypeCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
MutableExampleOfDroppedType ExampleOfDroppedTypeMutableCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<ExampleOfDroppedTypeObj>((*m_collection)[m_index]);
  return m_object;
}
...
ExampleOfDroppedTypeMutableCollectionIterator& ExampleOfDroppedTypeMutableCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
void ExampleOfDroppedTypeCollection::print(std::ostream& os, bool flush) const {
  os << *this;
  if (flush) {
    os.flush();
  }
